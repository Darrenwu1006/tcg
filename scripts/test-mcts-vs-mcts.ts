/**
 * MCTS vs MCTS Matchup Test Script
 * Tests Aoba Johsai Fast Attack deck against various other decks using MCTS for BOTH sides.
 */
...
async function runBattle(
  ...
  return runBattleWithOrder(ai1, ai2, deck1, deck2, "me", matchName);
}
...
async function main() {
  ...
  // We use SUMMARY log level to avoid flooding the console,
  // but you can change to DETAILED to see decision making for specific matches.
  const mctsAI_Me = new MCTSAI("me", 200, MCTSLogLevel.SUMMARY);
  const mctsAI_Opponent = new MCTSAI("opponent", 200, MCTSLogLevel.SUMMARY);

  console.log(
    "\n=== Tournament: Aoba Johsai (MCTS) vs Others (MCTS) [Balanced Turns] ==="
  );
  ...
  for (const opp of opponents) {
    ...
    // Game 1: Me (Aoba Johsai) goes first
    console.log(`\n--- Match vs ${opp.name} (Me First) ---`);
    await runBattle(
      mctsAI_Me,
      mctsAI_Opponent,
      myDeck,
      oppDeck,
      `vs ${opp.name} (Me First)`
    );

    // Game 2: Opponent goes first
    // Note: When opponent goes first, 'me' is still Aoba Johsai, but 'opponent' starts.
    // The GameEngine constructor takes (deck1, deck2, firstPlayer).
    // deck1 is always 'me's deck, deck2 is 'opponent's deck.
    console.log(`\n--- Match vs ${opp.name} (Opponent First) ---`);
    await runBattleWithOrder(
      mctsAI_Me,
      mctsAI_Opponent,
      myDeck,
      oppDeck,
      "opponent",
      `vs ${opp.name} (Opponent First)`
    );
  }
}
